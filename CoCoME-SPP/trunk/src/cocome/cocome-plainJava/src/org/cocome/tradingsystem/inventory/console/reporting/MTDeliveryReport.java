 ...
 */
final class MTDeliveryReport extends JPanel {

	public MTDeliveryReport(final RemoteComponent<IReporting> remoteReporting) {
				...
				"Create Report", new ActionListener() {
					...
					public void actionPerformed(@SuppressWarnings("unused") final ActionEvent event) {
						try {
							...
							final ReportTO report = reporting.getEnterpriseDeliveryReport(enterpriseId);
							...
							statusLabel.setText("Invalid enterprise id: " + e.getMessage());
							...
							statusLabel.setText(
									"Failed to get enterpise delivery report: " + e.getMessage()
									);
						}
					}
				}
