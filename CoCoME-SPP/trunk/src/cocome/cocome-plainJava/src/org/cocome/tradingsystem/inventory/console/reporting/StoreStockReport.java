 ...
 */
final class StoreStockReport extends JPanel {

	public StoreStockReport(final RemoteComponent<IReporting> remoteReporting) {
		super(new BorderLayout());

		//

		final JLabel hintLabel = new JLabel("Enter store id and press 'Create Report'");
				...
				"Create Report", new ActionListener() {
					...
					public void actionPerformed(@SuppressWarnings("unused") final ActionEvent event) {
						try {
							...
							final long storeId = Long.parseLong(inputField.getText());
							final IReporting reporting = remoteReporting.get();
							final ReportTO report = reporting.getStoreStockReport(storeId);
							...
							statusLabel.setText("Invalid store id: " + nfe.getMessage());

						} catch (final EntityNotFoundException enfe) {
							statusLabel.setText("There is no such store!");

						} catch (final Exception e) {
							statusLabel.setText(
									"Failed to get store stock report: " + e.getMessage()
									);
						}
					}
				}
